...
| BASE-004 | Open | Provider contracts | Jellyfin and Emby do not yet have recorded contract coverage equivalent to the strongest Plex client coverage. | Missing coverage | High | Conditional | Provider boundary | M01 PR 01F or explicit deferral | `characterization-matrix.md` |
| BASE-005 | Open | Scheduling determinism | Same-input, same-seed, stable-order schedule fixtures are incomplete. | Missing coverage | Critical | Conditional | Scheduling boundary | M01 PR 01F or explicit deferral | `characterization-matrix.md` |
| BASE-006 | Open | Output contracts | M3U and HDHomeRun-compatible golden contract coverage is incomplete. | Missing coverage | High | Conditional | Output boundary | M01 PR 01F or explicit deferral | `characterization-matrix.md` |
| BASE-007 | In progress | Change control | The inherited contribution guide and repository pull-request template do not define ChannelForge change-control requirements. | Documentation gap | Moderate | Yes | Project governance | M01 PR 01E | This pull request |
